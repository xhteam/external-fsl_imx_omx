        ...
        OMX_STATETYPE eState)
{
    ...
    LOG_DEBUG("%s state trans from %d to %d\n", name, eCurState, eState);
...
OMX_ERRORTYPE GMComponent::WaitTunneledPortDisable(
        OMX_U32 nPortIndex)
{
    WaitCommand(OMX_CommandPortDisable, nPortIndex, NULL, OMX_TICKS_PER_SECOND);
    PortsInfo[nPortIndex].bEnabled = OMX_FALSE;
    return OMX_ErrorNone;
}
...
OMX_ERRORTYPE GMComponent::WaitTunneledPortEnable(
        OMX_U32 nPortIndex)
{
    WaitCommand(OMX_CommandPortEnable, nPortIndex, NULL, OMX_TICKS_PER_SECOND);
    PortsInfo[nPortIndex].bEnabled = OMX_TRUE;
    return OMX_ErrorNone;
}
        ...
        case OMX_EventPortSettingsChanged:
            {
                ...
                if(PortsInfo[nPortIndex].bFisrtPortSettingChanged){
                    LOG_DEBUG(" the first port setting change \r\n");
                    OMX_INIT_STRUCT(&sPortDef, OMX_PARAM_PORTDEFINITIONTYPE);
                    ...
                    PortsInfo[nPortIndex].bFisrtPortSettingChanged=OMX_FALSE;
                    fsl_osal_cond_signal(Cond);
                }
                else{
                    LOG_DEBUG("non-first port setting change \r\n");
                    if(SysEventHandler)    (*SysEventHandler)(this, SysEventAppData, eEvent, nData1, nData2, pEventData);	
                }				
            }
            break;
        case OMX_EventError:
        ...
        OMX_BUFFERHEADERTYPE* pBuffer)
{
    ...
    if(PortsInfo[nPortIndex].bOutputed != OMX_TRUE) {
...
OMX_ERRORTYPE GMComponent::DynamicPortSettingChange(OMX_U32 nPortIndex)
{
    OMX_ERRORTYPE ret = OMX_ErrorNone;
    OMX_PARAM_PORTDEFINITIONTYPE sPortDef1, sPortDef2;
    ...
    LOG_DEBUG("%s: port index: %d, allocator: %d \r\n",__FUNCTION__,nPortIndex,PortsInfo[nPortIndex].Allocater);
    pPeer = PortsInfo[nPortIndex].peer.pComponent;
    nPeerPortIndex = PortsInfo[nPortIndex].peer.nPortIndex;
    LOG_DEBUG("peer name: %s, nPeerPortIndex: %d \r\n",pPeer->name,nPeerPortIndex);

    //disable port
    if(PortsInfo[nPortIndex].Allocater==GM_SELF_ALLOC){
        ret=pPeer->PortDisable(nPeerPortIndex);    if(ret != OMX_ErrorNone) {return ret;}
        ret=PortDisable(nPortIndex);    if(ret != OMX_ErrorNone) {return ret;}
    }
    else if(PortsInfo[nPortIndex].Allocater==GM_PEER_ALLOC){
        ret=PortDisable(nPortIndex);    if(ret != OMX_ErrorNone) {return ret;}
        ret=pPeer->PortDisable(nPeerPortIndex);    if(ret != OMX_ErrorNone) {return ret;}
        ...
        LOG_ERROR("%s: unsupported allocater %d \n",__FUNCTION__,PortsInfo[nPortIndex].Allocater);
        return OMX_ErrorNotImplemented;
    }

    //re-set  peer's port/crop info
    OMX_INIT_STRUCT(&sPortDef1, OMX_PARAM_PORTDEFINITIONTYPE);
    sPortDef1.nPortIndex = nPortIndex;
    OMX_GetParameter(hComponent, OMX_IndexParamPortDefinition, &sPortDef1);
    ...
    OMX_INIT_STRUCT(&sCrop, OMX_CONFIG_RECTTYPE);
    sCrop.nPortIndex=nPortIndex;
    OMX_GetConfig(hComponent, OMX_IndexConfigCommonOutputCrop, &sCrop);
    LOG_DEBUG("%s: video: [left,top,width,height]=[%d, %d, %d, %d] , width: %d, height: %d \r\n",__FUNCTION__, sCrop.nLeft,sCrop.nTop,sCrop.nWidth,sCrop.nHeight,sPortDef1.format.video.nFrameWidth,sPortDef1.format.video.nFrameHeight);
    sCrop.nPortIndex=nPeerPortIndex;
    OMX_SetConfig(pPeer->hComponent,OMX_IndexConfigCommonInputCrop,&sCrop);

    //port enable
    if(PortsInfo[nPortIndex].Allocater==GM_SELF_ALLOC){
        ret=PortEnable(nPortIndex);    if(ret != OMX_ErrorNone) {return ret;}
        ret=pPeer->PortEnable(nPeerPortIndex);    if(ret != OMX_ErrorNone) {return ret;}
    }
    else if(PortsInfo[nPortIndex].Allocater==GM_PEER_ALLOC){
        ret=pPeer->PortEnable(nPeerPortIndex);    if(ret != OMX_ErrorNone) {return ret;}
        ret=PortEnable(nPortIndex);    if(ret != OMX_ErrorNone) {return ret;}
    ...
    OMX_GetState(hComponent, &eState);
    if(eState == OMX_StateExecuting) {
        LOG_DEBUG("Fill buffers: cnt: %d  \r\n",PortsInfo[nPortIndex].nBuffers);
        ...
        OMX_U32 nPortIndex)
{
    ...
    switch(sPortDef.eDomain) {
        ...
        case OMX_PortDomainVideo:
            switch((int)(sPortDef.format.video.eCompressionFormat)) {
