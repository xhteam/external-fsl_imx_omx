...
#define ONE_MEGA_BYTES (1024*1024)
...
typedef struct _SHARED_FD_PIPE
{
    ...
    CPint nWriteBufferLen;
    CPhandle pWriteBuffer;
...
OMX_SFD_PIPE_API CPresult SharedFdPipe_Open(CPhandle* hContent, CPstring szURI, CP_ACCESSTYPE eAccess)
{
    ...
    hPipe->pWriteBuffer = NULL;
    if (hPipe->eAccess == CP_AccessWrite) {
        hPipe->nWriteBufferLen = ONE_MEGA_BYTES;
        hPipe->pWriteBuffer = FSL_MALLOC(hPipe->nWriteBufferLen);
        fsl_osal_memset(hPipe->pWriteBuffer, 0, hPipe->nWriteBufferLen);
    }
...
OMX_SFD_PIPE_API CPresult SharedFdPipe_Close(CPhandle hContent)
{
    ...
    if (hPipe->pWriteBuffer)
        FSL_FREE(hPipe->pWriteBuffer);
        ...
        CPhandle  hContent, CPint64 nOffset, CP_ORIGINTYPE eOrigin)
{
    ...
    if( nPos > hPipe->nBeginPoint + hPipe->nLen || nPos < hPipe->nBeginPoint) {
		if (hPipe->eAccess == CP_AccessWrite \
				&& nPos > hPipe->nBeginPoint + hPipe->nLen) {
            // As ftruncate64() can't consume storage space on ext4 file system. (
            // can consume storage space on vfat.) Use write function to implement it.
            ...
            if (nNeedWriteLen > hPipe->nWriteBufferLen) {
                if (nNeedWriteLen <= hPipe->nWriteBufferLen + ONE_MEGA_BYTES) {
                    hPipe->nWriteBufferLen += ONE_MEGA_BYTES;
                    hPipe->pWriteBuffer = FSL_REALLOC(hPipe->pWriteBuffer, hPipe->nWriteBufferLen);
                    fsl_osal_memset(hPipe->pWriteBuffer, 0, hPipe->nWriteBufferLen);
                } else {
                    hPipe->nWriteBufferLen = nNeedWriteLen;
                    hPipe->pWriteBuffer = FSL_REALLOC(hPipe->pWriteBuffer, hPipe->nWriteBufferLen);
                    fsl_osal_memset(hPipe->pWriteBuffer, 0, hPipe->nWriteBufferLen);
