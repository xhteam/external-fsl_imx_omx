...
PcmDec::PcmDec()
{
    fsl_osal_strcpy((fsl_osal_char*)name, "OMX.Freescale.std.audio_decoder.pcm.sw-based");
    ComponentVersion.s.nVersionMajor = 0x1;
    ComponentVersion.s.nVersionMinor = 0x1;
    ComponentVersion.s.nRevision = 0x2;
    ComponentVersion.s.nStep = 0x0;
    role_cnt = 1;
    role[0] = (OMX_STRING)"audio_decoder.pcm";
...
OMX_ERRORTYPE PcmDec::InitComponent()
{
...
OMX_ERRORTYPE PcmDec::DeInitComponent()
{
    return OMX_ErrorNone;
}

OMX_ERRORTYPE PcmDec::AudioFilterInstanceInit()
{
    OMX_ERRORTYPE ret = OMX_ErrorNone;
...
OMX_ERRORTYPE PcmDec::AudioFilterCodecInit()
{
    OMX_ERRORTYPE ret = OMX_ErrorNone;
...
OMX_ERRORTYPE PcmDec::AudioFilterInstanceDeInit()
{
    OMX_ERRORTYPE ret = OMX_ErrorNone;
...
OMX_ERRORTYPE PcmDec::AudioFilterGetParameter(
...
OMX_ERRORTYPE PcmDec::AudioFilterSetParameter(
        OMX_INDEXTYPE nParamIndex, 
        OMX_PTR pComponentParameterStructure)
{
    OMX_ERRORTYPE ret = OMX_ErrorNone;
...
OMX_ERRORTYPE PcmDec::AudioFilterSetParameterPCM()
{
    OMX_ERRORTYPE ret = OMX_ErrorNone;
...
void PcmDec::PcmProcess(OMX_U8 *pOutBuffer, OMX_U32 *pOutLen, OMX_U8 *pInBuffer, OMX_U32 nInLen)
{
	OMX_U32 i, j, Len;
...
AUDIO_FILTERRETURNTYPE PcmDec::AudioFilterFrame()
{
	...
	LOG_DEBUG("PcmDec process frame len: %d\n", nActuralLen);
...
OMX_ERRORTYPE PcmDec::AudioFilterReset()
{
    OMX_ERRORTYPE ret = OMX_ErrorNone;
...
OMX_ERRORTYPE PcmDec::AudioFilterCheckCodecConfig()
{
	OMX_ERRORTYPE ret = OMX_ErrorNone;
	...
	LOG_DEBUG("Get PCM decoder config. Channel: %d, Sample Rate: %d, BitsPerSample: %d\n", \
...
extern "C" {
    OMX_ERRORTYPE PcmDecInit(OMX_IN OMX_HANDLETYPE pHandle)
    {
        OMX_ERRORTYPE ret = OMX_ErrorNone;
        PcmDec *obj = NULL;
