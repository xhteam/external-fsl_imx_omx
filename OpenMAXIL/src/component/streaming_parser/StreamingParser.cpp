        ...
        OMX_BUFFERHEADERTYPE *pBufferHdr)
{
    ...
    return DumpDataToOMXBuffer(pkt, pBufferHdr);
}
...
OMX_ERRORTYPE StreamingParser::ReadPacketFromStream()
{
    ...
    if(pkt->flags & AV_PKT_FLAG_DISCONTINUE)
        HandleStreamingSkip();
...
{
    //OMX_TICKS CurMediaTime = GetCurrentMediaTime();
    OMX_TICKS CurMediaTime = 0;
    OMX_TICKS ACacheDepth = pktCache[AUDIO_OUTPUT_PORT].GetCacheDepth(CurMediaTime);
    OMX_TICKS VCacheDepth = pktCache[VIDEO_OUTPUT_PORT].GetCacheDepth(CurMediaTime);
...
OMX_TICKS StreamingParser::PktCache::GetCacheDepth(
        OMX_TICKS CurMediaTime)
{
    return ABS(LastPktTs - (CurMediaTime > 0 ? CurMediaTime : FirstPktTs));
}
