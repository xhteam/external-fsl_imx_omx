...
#if 0
    {
        FILE *fp = fopen("/dump/video.bit", "ab");
        if(fp) {
            fwrite(pBuffer, 1, nSize, fp);
            fclose(fp);
        }
    }
#endif
...
OMX_ERRORTYPE SorensonDec::GetOutputBuffer(OMX_PTR *ppBuffer,OMX_S32* pOutSize)
{
    ...
    LOG_DEBUG("%s,%d, buffer got %x size: %d\n",__FUNCTION__,__LINE__,pOutBuffer, nOutBufferSize);
    ...
    *pOutSize = nOutBufferSize;
	...
	// Set frame to black if it is invalid frame to fix thumbnail issue.
	if (*pOutSize == 0) {
		...
		fsl_osal_memset(v_addr, 128, v_addr - u_addr);
	}
...
#if 0
    {
        printf("info.w: %d, info.h: %d\n", Info.width, Info.height);
        int size = Info.width * Info.height * 3/2;
        FILE *fp = fopen("/dump/video1.yuv", "ab");
        if(fp) {
            fwrite(pOutputFrame, 1, size, fp);
            fclose(fp);
        }
    }
#endif
